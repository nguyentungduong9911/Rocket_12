...
CREATE OR REPLACE VIEW view_DSNV_Sale AS 
SELECT A.*,D.DepartmentName  
FROM `Account` A
INNER JOIN Department D ON A.DepartmentID = D.DepartmentID
WHERE D.DepartmentName = "Sale";
SELECT * FROM view_DSNV_Sale;
...
CREATE OR REPLACE VIEW view_In4accountmaxgroup
AS
...
LIMIT		1);
SELECT 	* FROM 	view_In4accountmaxgroup;
...
CREATE OR REPLACE VIEW  View_Contenttren18tu AS
SELECT LENGTH (Content)
FROM Question
WHERE LENGTH(Content) > 18;
SELECT * FROM View_Contenttren18tu;
...
CREATE OR REPLACE VIEW view_QuestionHoNguyen AS
SELECT Q.*, A.Fullname
FROM Question Q
INNER JOIN `Account` A ON Q.CreatorID = A.AccountID	
WHERE SUBSTRING_INDEX (Fullname,'',1) ='Nguyen';
SELECT 	* FROM 	view_QuestionHoNguyen;

-- Chạy bằng CTE
WITH QuestionHoNguyen AS (
SELECT Q.*, A.Fullname
FROM Question Q
INNER JOIN `Account` A ON Q.CreatorID = A.AccountID
WHERE SUBSTRING_INDEX(Fullname,' ',1)='Nguyen')
SELECT * FROM QuestionHoNguyen
